...
crontab -l | { cat; echo "5,15,25,35,45,55 * * * * . \$HOME/.profile; cd $HOME/python_projects/IBKR && python3 jobs/check_connection_status.py >> $HOME/logs/check_connection_status/\`date +\%Y-\%m-\%d_\%H-\%M-\%S\`.log 2>&1"; } | crontab -
